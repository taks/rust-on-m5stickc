...
if [ "${WOKWI_PROJECT_ID}" == "" ]; then
    wokwi-server --chip esp32 target/${ESP_ARCH}/${BUILD_MODE}/rust-on-m5stickc
else
    wokwi-server --chip esp32 --id ${WOKWI_PROJECT_ID} target/${ESP_ARCH}/${BUILD_MODE}/rust-on-m5stickc
fi